...
** emacs-scratch-frame.ps1

*Purpose:* Create, focus, or minimize the tiled Howm scratch Emacs frame.

*Trigger:* =LWin+Shift+S= (Kanata =lmet-launcher=).

#+begin_src powershell :tangle kanata/emacs-scratch-frame.ps1
...
** wtq-howm-launcher.ps1

*Purpose:* Launch the dedicated Howm scratch frame for WTQ.

*Trigger:* =LWin+s= in WTQ (not Kanata). Creates the frame if it doesn't exist; exits silently if it does.

#+begin_src powershell :tangle kanata/wtq-howm-launcher.ps1
...
$credential = (New-Object Windows.Security.Credentials.PasswordVault).Retrieve("gmail", "***@***.***")  # literal vault key (not redacted)
...
*Purpose:* Commit and push all three repos if there are pending changes. Skips any repo with no changes.

*Schedule:* Windows Task Scheduler task =AutoGitSync= (=\AutoGitSync=), every 2 hours, runs =pwsh.exe= directly — no Claude dependency.

| Repo | Method | Commit prefix |
|------+--------+---------------|
| =~/.config= dotfiles | =yadm add -u= | =auto: dotfiles= |
| =~/Documents/org= notes | =git add -A= | =auto: notes= |
| =~/basic-memory= | =git add -A= | =auto: basic-memory= |

#+begin_src powershell :tangle utility/auto-git-sync.ps1
# auto-git-sync.ps1 — commit and push all three repos if there are changes.
# Runs on a schedule (see Task Scheduler: AutoGitSync).

$stamp = Get-Date -Format "yyyy-MM-dd HH:mm"

function Sync-Repo {
    param([string]$Path, [string]$Label, [bool]$UseYadm = $false)

    Push-Location $Path
    try {
        if ($UseYadm) {
            $changes = yadm status --porcelain 2>$null
            if ($changes) {
                yadm add -u
                yadm commit -m "auto: $Label $stamp"
                yadm push
            }
        } else {
...
Sync-Repo -Path "$env:USERPROFILE\.config"      -Label "dotfiles"     -UseYadm $true
Sync-Repo -Path "$env:USERPROFILE\Documents\org" -Label "notes"
Sync-Repo -Path "$env:USERPROFILE\basic-memory"  -Label "basic-memory"
#+end_src
...
if (Test-Path $daemonPs1) {
    Start-Process "pwsh.exe" -ArgumentList "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$daemonPs1`"" -WindowStyle Hidden
}
#+end_src
...
*Purpose:* Start the Ollama server silently in the background.

*Trigger:* Called from startup or manually. No window shown.

#+begin_src powershell :tangle ai/start-ollama.ps1 :mkdirp yes
...
*Purpose:* Launch a NixOS QEMU VM via WSL Ubuntu. 8 vCPU / 8 GB RAM / UEFI / VNC at =localhost:5900=.

*Trigger:* Manual — run from PowerShell when needed.

#+begin_src powershell :tangle nixos-vm-launch.ps1 :mkdirp yes
$WslDistro = "Ubuntu"
$VmDir = "/home/namef/nixos-vm"

Write-Host "=== NixOS VM Launcher ===" -ForegroundColor Cyan
Write-Host "Starting QEMU VM (4 vCPU, 8GB RAM, UEFI)..." -ForegroundColor Yellow
Write-Host "VNC available at localhost:5900 -> connect via TightVNC Viewer" -ForegroundColor Green

wsl -d $WslDistro bash -c @"
cd $VmDir
cp /usr/share/OVMF/OVMF_VARS_4M.fd nixos-vars.fd 2>/dev/null
qemu-system-x86_64 \
  -machine q35 \
  -cpu max \
  -smp 4 \
  -m 8G \
  -drive file=nixos-disk.qcow2,if=virtio,format=qcow2 \
  -cdrom nixos-graphical-26.05-x86_64.iso \
  ...
  -drive if=pflash,format=raw,file=nixos-vars.fd
"@
