# Create, focus, or minimize the tiled Howm scratch Emacs frame.
# Bound in the Kanata lmet-launcher layer to LWin + Shift + S.
...
$status = & $emacsClient --eval '(my/howm-frame-status)' 2>$null
...
if ($status -notmatch '"(?:selected|exists)"') {
    ...
    Start-Process -FilePath $emacsClientW `
        -ArgumentList '--alternate-editor= -c -n -F "((name . \"Howm Scratch Tiled\") (title . \"Howm Scratch Tiled\"))" --eval "(run-with-timer 0 nil (lambda () (my/howm-frame-setup)))"' `
...
if ($emacsIsFg -and $status -match '"selected"') {
    & $emacsClient --eval '(my/howm-minimize)' 2>$null
} else {
    if ($emacsProc) { [WinFocus]::AllowSetForegroundWindow($emacsProc.Id) | Out-Null }
    & $emacsClient --eval '(my/howm-raise)' 2>$null
    Set-EmacsFrameForeground -TitleEquals 'Howm Scratch Tiled'
}
